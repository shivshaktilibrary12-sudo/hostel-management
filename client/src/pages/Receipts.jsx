...
export default function Receipts() {
  ...
  const handlePackageChange = async (pkg) => {
    setForm(p => ({ ...p, packageName: pkg }));
    if (pkg === 'final' && form.roomNumber) {
      // Auto-calculate cumulative bill for this room
      try {
        const rRes = await receiptsAPI.getAll({ room: form.roomNumber, limit: 500 });
        const rList = rRes.data?.data || rRes.data || [];
        // Sum all outstanding balance dues from part payments
        ...
        // Total rent collected so far
        const rentCollected = rList.filter(r => r.packageName === 'rent').reduce((s, r) => s + (r.amountPaid || r.totalAmount || 0), 0);
        const elecCollected = rList.filter(r => r.packageName === 'electric').reduce((s, r) => s + (r.amountPaid || r.totalAmount || 0), 0);
        const advCollected  = rList.filter(r => r.packageName === 'advance').reduce((s, r) => s + (r.amountPaid || r.totalAmount || 0), 0);
        const cumTotal = pendingBalance + fixedRent;
        const noteStr = [
          fixedRent    ? `Rent: ₹${fixedRent}`          : '',
          pendingBalance > 0 ? `Pending dues: ₹${pendingBalance}` : '',
        ].filter(Boolean).join(' + ');
        setForm(p => ({
          ...p,
          packageName: pkg,
          totalAmount: String(cumTotal || ''),
          notes: `Final bill: ${noteStr}. Total rent collected so far: ₹${rentCollected}. Electric: ₹${elecCollected}.`,
        }));
      } catch(e) {
        setForm(p => ({ ...p, packageName: pkg }));
      }
    } else if (roomConfig) {
      const amount = pkg === 'rent'    ? String(roomConfig.rent    || 0)
                   : pkg === 'advance' ? String(roomConfig.advance || 0)
                   : '';
      setForm(p => ({ ...p, packageName: pkg, totalAmount: amount }));
    }
  };
  ...
  const handleRoomChange = async (roomNo) => {
    setForm(p => ({ ...p, roomNumber: roomNo, fromDate: '', toDate: '', totalAmount: '' }));
    setRoomConfig(null);
    ...
    try {
      ...
      // Auto-fill fromDate from last receipt's toDate for this room
      const roomReceipts = receipts
        .filter(r2 => String(r2.roomNumber) === String(roomNo) && r2.toDate)
      ...
      setForm(p => ({ ...p, roomNumber: roomNo, totalAmount: amt, fromDate }));
    } catch(e) {}
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      // Fix 1: Auto-update roomLeavingDate for all members in the room
      // This removes them from the overdue list automatically
